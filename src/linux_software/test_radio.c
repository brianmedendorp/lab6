...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 8192; // loops 2048 times, each read is 32bits (4 bytes) 
    float time_spent = (stop_time - start_time)*(8*pow(10,-9)) ; //125MHz clock period 
    float throughput= (bytes_transferred/time_spent)*pow(10,-6);
    printf("You transferred %i bytes of data in %f seconds\n",bytes_transferred,time_spent);
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Brian Medendorp - Custom Peripheral Demonstration\n\r");
